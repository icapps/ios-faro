        ...
        describe("MockService responses") {
            ...
            context("error cases") {
                it("HttpError when statuscode > 400") {
					...
					let result = service.handle(data: nil, urlResponse: response, error: nil, for: URLRequest(url: URL(fileURLWithPath: ""))) as Result<MockModel>
                    switch result {
                ...
                it("Fail for NSError") {
					...
					let result = service.handle(data: nil, urlResponse: nil, error: nsError, for: URLRequest(url: URL(fileURLWithPath: ""))) as Result<MockModel>
                    switch result {
                    case .failure(let faroError):
                        switch faroError {
                        case .nonFaroError(_):
...
class ExpectResponse {
    static func statusCode(_ statusCode: Int, data: Data? = nil, service: Service) {
		...
		let result = service.handle(data: data, urlResponse: response, error: nil, for: URLRequest(url: URL(fileURLWithPath: ""))) as Result<MockModel>
        if let data = data {
            switch result {
            case .data(_):
                break
