    ...
    override func spec() {
        ...
        describe("Call with parameters") {
            ...
            it("should insert http headers into the request") {
                let params = Parameters(type: Parameters.ParameterType.httpHeader, parameters: ["Accept-Language" : "en-US",
            ...
            it("should fail to insert http headers that arent strings into the request") {
                let params = Parameters(type: Parameters.ParameterType.httpHeader, parameters: ["Accept-Language" : 12345,
            ...
            it("should insert URL components into the request") {
                let params = Parameters(type: Parameters.ParameterType.urlComponents, parameters: ["some query item": "aa🗿🤔aej"])
                let call = Call(path: "path", method: .GET, rootNode: nil, parameters: params)
                let request = call.request(withConfiguration: configuration)
            ...
            it("should fail to insert URL components that arent strings into the request") {
                let params = Parameters(type: Parameters.ParameterType.urlComponents, parameters: ["some dumb query item": Data(base64Encoded: "el wrongo")])
                let call = Call(path: "path", method: .GET, rootNode: nil, parameters: params)
                let request = call.request(withConfiguration: configuration)
            ...
            it("should add JSON into the request body into the request") {
                let params = Parameters(type: Parameters.ParameterType.jsonBody, parameters: ["a string": "i am a string. well met.",
                                                                                              "a number": 123])
            ...
            it("should fail to add JSON into a GET or DELETE request") {
                let params = Parameters(type: Parameters.ParameterType.jsonBody, parameters: ["a string": "good day i am a string",
                                                                                              "a number": 123])
                let call = Call(path: "path", method: .GET, rootNode: nil, parameters: params)
                let data = call.request(withConfiguration: configuration)?.httpBody
